...
const createMarketsShippingTablesSQL = `
-- ========================================
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'markets_name_store_unique') THEN
        ALTER TABLE public.markets ADD CONSTRAINT markets_name_store_unique UNIQUE(store_id, name);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'shipping_methods_name_market_unique') THEN
...
DROP POLICY IF EXISTS "Anyone can read shipping addresses" ON public.shipping_addresses;
...
export async function setupMarketsDatabase() {
  try {
    console.log('🚀 Configuration des tables Markets & Shipping...');

    // Créer la table markets
    const { error: marketsError } = await supabase.rpc('create_markets_table');
    if (marketsError && !marketsError.message.includes('already exists')) {
      console.error('❌ Erreur lors de la création de la table markets:', marketsError);
    }

    // Créer la table shipping_methods
    const { error: methodsError } = await supabase.rpc('create_shipping_methods_table');
    if (methodsError && !methodsError.message.includes('already exists')) {
      console.error('❌ Erreur lors de la création de la table shipping_methods:', methodsError);
    }

    // Créer la table shipping_addresses
    const { error: addressesError } = await supabase.rpc('create_shipping_addresses_table');
    if (addressesError && !addressesError.message.includes('already exists')) {
      console.error('❌ Erreur lors de la création de la table shipping_addresses:', addressesError);
    }

    console.log('✅ Tables créées avec succès');

    // Vérifier que les tables existent
    const { data: markets, error: marketsCheckError } = await supabase
      .from('markets')
      .select('count')
      .limit(1);

    if (marketsCheckError) {
      console.error('❌ Erreur lors de la vérification de la table markets:', marketsCheckError);
      throw marketsCheckError;
    }

    const { data: shippingMethods, error: methodsCheckError } = await supabase
      .from('shipping_methods')
      .select('count')
      .limit(1);

    if (methodsCheckError) {
      console.error('❌ Erreur lors de la vérification de la table shipping_methods:', methodsCheckError);
      throw methodsCheckError;
    }
...
// Fonction pour exécuter le setup si ce fichier est appelé directement
if (typeof window !== 'undefined') {
  (window as any).setupMarketsDatabase = setupMarketsDatabase;
}
